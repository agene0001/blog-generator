...
const InteractiveSection: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const animation = anime.timeline({
            loop: false, easing: 'easeOutExpo',
        });

        chars.forEach((char, index) => {
            animation.add({
                targets: inputRef.current, value: inputRef.current?.value + char, delay: index * 100, duration: 100
            });
        });
    ...
    function triggerAnimation() {
        ...
        if (textWrapper) {
            ...
            const animation = anime.timeline({loop: false}).add({
                ...
                .add({
                    targets: '.ml6', opacity: 0, duration: 1000, easing: "easeOutExpo", complete: function () {
                        animation.restart();
                        animation.pause()
                    }, delay: 1000,
    ...
    return (<>
        ...
        <div className="flex justify-center">
            <div className=" sm:m-4 md:ml-80 p-4 rounded-md jetbrains-mono-Uni" style={{backgroundColor: "#16213B"}}>

                <span className="ml6 letters text-amber-50 "> Top 5 Tourist Attractions in New York City<br/></span>
                <span className="text-wrapper md:ml-72 lg:m-0 ">
              <span className="letters text-amber-50 ">
                        ...
                        <div className="ml5 text-amber-50">1. The Statue of Liberty and Ellis Island</div>
